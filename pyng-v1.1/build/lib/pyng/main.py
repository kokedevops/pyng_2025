...
from pyng import db, app, scheduler, config
from pyng.api import get_web_themes, get_polling_config, get_active_theme
from pyng.database import Polling, WebThemes
from pyng.forms import PollingConfigForm, UpdatePasswordForm, UpdateEmailForm
from pyng.polling import update_poll_scheduler, add_poll_history_cleanup_cron
from pyng.alertas import update_host_status_alert_schedule
...
def favicon():
    '''Favicon'''
    return send_from_directory(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'static'), 'favicon.ico',
